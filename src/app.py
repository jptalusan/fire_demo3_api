...
# Global variable to store components (loaded lazily)
components = None

def get_prediction_components():
    """Lazy loading of prediction system components"""
    global components
    if components is None:
        try:
            models_dir = Path(__file__).parent.parent / "models" / "incident_prediction_system"
            components = load_incident_prediction_system(str(models_dir))
        except Exception as e:
            print(f"Warning: Could not load prediction system: {e}")
            components = {}
    return components
...
def predict_incidents_with_types_and_coordinates(start_date, end_date):
    """
    ...
    components = get_prediction_components()
    if not components:
...
async def generate_incidents(request: Request):
    ...
    try:
        ...
        # Define paths
        data_dir = Path(__file__).parent.parent / "data"
        query_dir = data_dir / "incidents" / "synthetic" / "query"  # Use synthetic directory
        query_path = query_dir / query_filename
        ...
        if query_path.exists():
            ...
            print(f"Creating new synthetic query: {query_filename}")

            # Generate random incidents using the prediction system
            predicted_incidents_df = predict_incidents_with_types_and_coordinates(start_date, end_date)
